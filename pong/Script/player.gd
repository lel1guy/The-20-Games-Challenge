...
func _process(delta):
	if Input.is_action_pressed("PlayerUp"): #Whyle Up is being presses it moves ussing the paddle_speed var from the parent node "Main" * delta
		position.y -= get_parent().paddle_speed * delta
	elif Input.is_action_pressed("PlayerDown"): #Whyle Down is being presses it moves ussing the paddle_speed var from the parent node "Main" * delta
		position.y += get_parent().paddle_speed * delta
